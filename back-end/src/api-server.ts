...
export function initializeApiServer(service: KalshiService, minter: ChilizMinter) {
    ...
    app.get("/api/markets", async (req, res) => {
        try {
            const { limit, cursor, event_ticker, series_ticker, max_close_ts, min_close_ts, status, tickers } =
                req.query;
    ...
    app.get("/api/positions", async (req, res) => {
        try {
            ...
            // Note: position can be positive (YES) or negative (NO)
            const activePositions = positions.filter((pos: any) => {
                const totalPosition = pos.position || 0;
                const isActive = Math.abs(totalPosition) > 0;

                console.log(`Filtering ${pos.ticker}:`, {
                    totalPosition,
                    absolutePosition: Math.abs(totalPosition),
                    isActive,
                });
